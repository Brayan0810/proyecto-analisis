...
def coordenadas_juego():
    global direccion, posicion_play,x,y ,nueva_posicion,a 
    ...
    posicion_play = nueva_posicion + posicion_play[:-1]

    for parte, lugar in zip(canvas.find_withtag("snake"), posicion_play):
...
def movimiento():
    global posicion_food, posicion_play,nueva_posicion

    coordenadas_juego()

    cantidad['text'] = 'Coordenadas 🍎 : {}'.format(a)

    if posicion_play[0] == posicion_pared or posicion_play[0] == posicion_pared2 or posicion_play[0] == posicion_pared3 or posicion_play[0] == posicion_pared4:
        cruzar_snake()

    if posicion_food == posicion_play[0]:
        n = len(posicion_play)
        ...
        posicion_play.append(posicion_play[-1])

    
        if posicion_food not in posicion_play:
            canvas.coords(canvas.find_withtag("food"), posicion_food)

        canvas.create_text(*posicion_play[-1], text='O' , fill='green2', 
            font = ('Arial',30), tag ='snake')

    
        if posicion_play[-1] == nueva_posicion[0] and len(posicion_play)>=4: 
            cruzar_snake()


        for i in posicion_play:
            if len(posicion_play)==3:
                maximo_nivel()
